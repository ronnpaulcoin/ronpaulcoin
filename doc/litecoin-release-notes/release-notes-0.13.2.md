Ronpaulcoin Core version 0.13.2 is now available from:

  <https://download.ronpaulcoin.org/ronpaulcoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/ronpaulcoin-project/ronpaulcoin/issues>
...
updates will be released anymore. Without security updates, using a ronpaulcoin
...
ECDSA signatures inside Ronpaulcoin transactions now use validation using
...
Previous versions of Ronpaulcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Ronpaulcoin Core was to raise the default minimum
relay fee.

Ronpaulcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Ronpaulcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running ronpaulcoind without having to do any manual configuration.
...
Ronpaulcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Ronpaulcoin Core will relay transactions with
...
entirely in Ronpaulcoin Core 0.13, and it is at this time undecided whether the
...
Ronpaulcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Ronpaulcoin Core automatically creates a hidden service to listen on, without
manual configuration. Ronpaulcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Ronpaulcoin Core is listening, and
...
Ronpaulcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Ronpaulcoin
Core to use floating fees. By default, Ronpaulcoin Core will use floating
fees.
...
(default: `0.0002` RPC/kB).

At all times, Ronpaulcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) RPC.
Furthermore, Ronpaulcoin Core will never create transactions paying less than
...
`ronpaulcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /ronpaulcoinrpc>
        ...
        # optional bypass ronpaulcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< ronpaulcoinrpc:<password>
    </Location>
...
Restarting ronpaulcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in ronpaulcoin.conf
...
ronpaulcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/ronpaulcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Ronpaulcoin Core code base has
...
- `ronpaulcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `ronpaulcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Ronpaulcoin scripting system that in combination with
...
Ronpaulcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Ronpaulcoin Core.
...
The two phases are distinct in the Ronpaulcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Ronpaulcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Ronpaulcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Ronpaulcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of ronpaulcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of ronpaulcoins being spent is exactly
...
- **Increased security for multisig:** Ronpaulcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Ronpaulcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Ronpaulcoin's current ledger (called the UTXO set),
  ...
  note that Ronpaulcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Ronpaulcoin users to individually opt-in to almost any change in the Ronpaulcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Ronpaulcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Ronpaulcoin
client to Ronpaulcoin Core 0.13.2 to signal for segwit support. When a
...
Since Ronpaulcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `ronpaulcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `ronpaulcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
